...
execute as @a[scores={InfiniteEnderChest.LeaveGame = 1..},tag=!InfiniteEnderChest.Enable] run tellraw @s [{"text":"[InfiniteEnderChest]","color":"gold"},{"text":" この機能を利用するには、","color":"white"},{"text":"[こちら]","color":"aqua","clickEvent": {"action":  "run_command", "value": "/tag @s add InfiniteEnderChest.Enable"}, "hoverEvent": {"action": "show_text","contents": {"text": "有効にする"}}},{"text":" をクリックしてください。","color":"white"}]
execute as @a[scores={InfiniteEnderChest.LeaveGame = 1..},tag=!InfiniteEnderChest.Enable] run tellraw @s [{"text": "注意：この機能を利用すると、エンダーチェストの右上と右下のスロットが消えます。","color": "red"}]
